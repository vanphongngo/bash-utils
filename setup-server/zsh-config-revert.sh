...
backup_and_remove_user_zsh_files() {
  local u="$1"
  local home_dir
  # Read the home directory from passwd rather than `eval echo ~$u`, which
  # breaks on unusual usernames and evaluates whatever the name expands to.
  ...
  # Fix ownership in case the script runs as root (use the real primary group,
  # which is not always named after the user).
  local grp
  grp="$(id -gn "$u" 2>/dev/null || echo "$u")"
  chown -R "$u":"$grp" "$home_dir" || true
}
