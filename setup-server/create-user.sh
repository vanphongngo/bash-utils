...
# ---- 4. SSH key -------------------------------------------------------------
SSH_MODE="${SSH_MODE:-}"          # generate | paste | none
...
SSH access:
  1) generate — create a new ed25519 key pair for this user
  2) paste    — authorise a public key you already have
  3) none     — skip SSH setup
MENU
...
if [ "$SSH_MODE" = "paste" ] && [ -z "$SSH_PUBKEY" ]; then
  need_tty SSH_PUBKEY
  read -r -p "Paste the public key (ssh-ed25519 AAAA… / ssh-rsa AAAA…): " SSH_PUBKEY < /dev/tty || true
  [ -n "$SSH_PUBKEY" ] || { echo "❌ Empty public key."; exit 1; }
fi
...
# =============================================================================
echo
echo "→ user=$TARGET_USER privilege=$PRIVILEGE ssh=$SSH_MODE groups=${EXTRA_GROUPS:-<none>}"

# 5. Create the account. useradd -m (not adduser) because adduser is
...
case "$PRIVILEGE" in
  ...
  deploy)
    ...
    # BASH_SOURCE is /dev/stdin under `curl | bash`, so this only resolves when
    # the file was actually checked out.
    SCRIPT_DIR="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" 2>/dev/null && pwd || true)"
    if [ -n "$SCRIPT_DIR" ] && [ -f "$SCRIPT_DIR/deploy-sudoers.sh" ]; then
      TARGET_USER="$TARGET_USER" bash "$SCRIPT_DIR/deploy-sudoers.sh"
    else
      echo "⚠️  deploy-sudoers.sh not found next to this script — no deploy"
      echo "    rules were installed. Run it separately:"
      echo "    curl -fsSL <raw-url>/deploy-sudoers.sh | TARGET_USER=$TARGET_USER sudo -E bash"
    fi
    ;;
esac
...
if [ "$SSH_MODE" != "none" ]; then
  ...
  # sshd refuses to use these unless the permissions are exactly this tight.
  ...
  # Append only when absent, so a re-run does not duplicate the line.
  if ! grep -qxF "$SSH_PUBKEY" "$AUTH_KEYS"; then
    printf '%s\n' "$SSH_PUBKEY" >> "$AUTH_KEYS"
    echo "→ Authorised the key in $AUTH_KEYS"
  else
...
if [ "${SSH_MODE:-none}" = "generate" ]; then
  ...
  echo "Copy it to the machine you will connect FROM, then delete it here."
  if confirm "Print the private key now?"; then
    echo "---------------------------------------------------------------"
    cat "$HOME_DIR/.ssh/id_ed25519"
    echo "---------------------------------------------------------------"
    echo "Save it as ~/.ssh/${TARGET_USER}_ed25519, then: chmod 600 that file."
  fi
  echo
