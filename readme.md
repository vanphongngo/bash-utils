...
| `nodejs-playbook.md` | nvm / Node.js / Claude Code — **documentation, not a script** | n/a |
...
- `nodejs-playbook.md` (nvm, Node.js, Claude Code) is deliberately a document
  rather than an installer. Those tools live in `$HOME` and their setup needs an
  interactive shell: piped into `bash`, sudo has no reliable way to prompt for a
  password and fails before the first package is installed. Copy the blocks and
  run them as the user who will use the tools, never under `sudo`.
