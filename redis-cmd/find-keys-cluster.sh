...
# Redis Cluster Key Finder Script
# Finds all keys matching a pattern across multiple Redis instances
# Usage: ./find-keys-cluster.sh <redis-info-file> <pattern> [output-file]
...
USE_SCAN=false  # Set to true for production-safe SCAN instead of KEYS
...
# Read Redis instances from file (Bash 3.2+ compatible)
REDIS_INSTANCES=()
...
echo "======================================="
echo "Redis Cluster Key Finder"
echo "======================================="
echo -e "Pattern:       ${BLUE}${PATTERN}${NC}"
echo -e "Output file:   ${BLUE}${OUTPUT_FILE}${NC}"
echo -e "Instances:     ${BLUE}${TOTAL_INSTANCES}${NC}"
echo -e "Method:        ${BLUE}$([ "$USE_SCAN" = true ] && echo "SCAN (safe)" || echo "KEYS (fast)")${NC}"
echo "======================================="
echo ""

# Function to get keys using KEYS command (fast but blocks Redis)
get_keys_fast() {
    local host=$1
    local port=$2
    local pattern=$3
    ...
    if [ -n "$TIMEOUT_CMD" ]; then
        $TIMEOUT_CMD redis-cli -h "$host" -p "$port" KEYS "$pattern" 2>&1
    else
        redis-cli -h "$host" -p "$port" KEYS "$pattern" 2>&1
    fi
}

# Function to get keys using SCAN command (safe for production)
get_keys_safe() {
    local host=$1
    local port=$2
    local pattern=$3
...
for i in "${!REDIS_INSTANCES[@]}"; do
    ...
    # Get keys from this instance
    if [ "$USE_SCAN" = true ]; then
        RESULT=$(get_keys_safe "$host" "$port" "$PATTERN")
    else
        RESULT=$(get_keys_fast "$host" "$port" "$PATTERN")
    fi
    ...
    # Write keys to output file with instance prefix
    if [ -n "$RESULT" ]; then
